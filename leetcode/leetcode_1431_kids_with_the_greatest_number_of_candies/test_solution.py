...
ith kid has, and an integer extraCandies, denoting the number of
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            [2, 3, 5, 1, 3], 3, [True, True, True, False, True], id="example 1"
        ),
        pytest.param(
            [4, 2, 1, 1, 2], 1, [True, False, False, False, False], id="example 2"
        ),
    ],
)
def test_solution(fut, candies, extraCandies, expected):
    assert fut(candies, extraCandies) == expected
